...
def Save(event=None):
    ...
    try:
        ...
        stamp = datetime.now()
        dt = stamp.strftime('%Y-%m-%d %H:%M:%S') # year-month-date hour-minute-second
        transactionid = stamp.strftime('%Y%m%d%H%M%f') # %f is Microsecond
        dt = days[today]+'-'+dt # today will be changed as the current day

        with open('GUIExpensefile.csv','a',encoding='utf-8',newline='') as f:
            fw = csv.writer(f)
...
def read_csv():
    with open('GUIExpensefile.csv',newline='',encoding='utf-8') as f: # as f is name
        fr = csv.reader(f)
        data = list(fr)
    return data
...
header = ['Transactionid','Date-Time','Expense','Price','Quantity','Total']
...
def UpdateCSV():
    with open('GUIExpensefile.csv','w',newline='',encoding='utf-8') as f: # as f is name # 'w' is writer by replace new data
        fw = csv.writer(f)
        # prepare imformation from alltransaction to be list
...
def update_table():
    ...
    try: # detect error of none csv file
        data = read_csv()
        for d in data:
